...
class AuthController extends GetxController {
  ...
  Future<void> initSession() async {
    ...
    if (isLoggedIn) {
      ...
      final isAgree = prefs.getBool('isAgree') ?? true;
      final isDeleted = prefs.getBool('isDeleted') ?? false;
  ...
  Future<bool> loginAsUser(String username, String password) async {
    try {
      ...
      if (response.status) {
        ...
        await prefs.setBool('is_guest', userModel.value!.data.isGuest);
        await prefs.setBool('isAgree', userModel.value!.data.isAgree);
        await prefs.setBool('isDeleted', userModel.value!.data.isDeleted);
        ...
        if (errorMsg.contains("expired") ||
            errorMsg.contains("renew the membership")) {
          CustomSnackbars.failure(
            "Your membership has expired. Please renew it to regain access.",
            "Access Restricted",
          );
        } else if (errorMsg.contains("delete") ||
            errorMsg.contains("deleted")) {
          CustomSnackbars.failure(
            "Your account has been deleted. Please contact support if this was a mistake.",
            "Account Deleted",
          );
        } else {
      ...
      if (result.status == true) {
        ...
        userModel.value = LoginResponseModel(
          ...
          data: UserData(
            ...
            isGuest: true,
            isAgree: guestModel.value!.data!.isAgree ?? false,
            isDeleted: false,
          ),
        );
        ...
        await prefs.setBool('isAgree', guestModel.value!.data!.isAgree!);
        await prefs.setBool('isDeleted', false);
